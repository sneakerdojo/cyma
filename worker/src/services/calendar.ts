...
function buildEventDescription(intake: {
  ...
  const lines: string[] = [
    ...
    '',
    `**Service of interest:** ${escapeHtml(intake.selectedService)}`,
    `**Budget range:** ${escapeHtml(intake.budget)}`,
    `**Company:** ${escapeHtml(intake.contact.company) || '(not provided)'}`,
    '',
    '**Project details:**',
    escapeHtml(intake.requirements),
    '',
  ];

  if (intake.voiceNoteUrl) {
    lines.push(`**Voice note:** ${intake.voiceNoteUrl}`);
  }

  if (intake.attachmentUrl) {
    lines.push(`**Attachment:** ${intake.attachmentUrl}`);
  }

  lines.push('', '---', 'This event was auto-generated by the Octio booking agent.');
