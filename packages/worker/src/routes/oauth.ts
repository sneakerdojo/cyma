...
async function storeTokensForTenant(
  ...
  if (tenant === 'octio') {
    return await writeOctioRefreshTokenToEnv(tokens.refreshToken);
  }
...
async function writeOctioRefreshTokenToEnv(
  refreshToken: string,
): Promise<{ ok: boolean; message: string }> {
  ...
  try {
    ...
    return {
      ok: true,
      message:
        'Token written to worker/.env. Restart the worker to load it: bun run --filter @octio/worker dev',
    };
  } catch (err) {
