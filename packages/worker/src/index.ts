// config must be imported first so DATABASE_URL is set before db/client.ts
// reads it at module evaluation time. Bun auto-loads .env from CWD.
...
logger.info(
  { port: server.port, nodeEnv: config.nodeEnv },
  `Octio worker started on port ${server.port}`,
);
startCronJobs();
