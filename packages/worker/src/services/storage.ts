...
/** Resolve year/month subdirectory. Bun.write auto-creates parent dirs. */
function resolveUploadDir(baseDir: string, now: Date): string {
...
export async function saveVoiceNote(
  ...
  const now = new Date();
  const uploadDir = resolveUploadDir(baseDir, now);

  const uuid = crypto.randomUUID();
  const filename = `${uuid}-voice.webm`;
  const absolutePath = path.join(uploadDir, filename);

  await Bun.write(absolutePath, buf);
